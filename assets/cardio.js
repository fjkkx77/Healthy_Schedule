...
/* ---------- 1. 数字看风险 ---------- */
const stats = [
  { num: "54.4万", color: "var(--red)", label: "我国每年心源性猝死人数（<strong>估算</strong>）", note: "数据出身：源自 2005–2006 年四地区监测（发生率 41.84/10万，实测猝死 284 例），按当时 13 亿人口推算。这是国内被引用最广的数字，但已有 20 年，且近 5 年《中国心血管健康与疾病报告》未再收录猝死数据——当作量级参考，别当作当年实测值" },
  { num: "约80%", color: "var(--blue)", label: "缺血性脑卒中占全部脑卒中比例", note: "即脑梗死，是卒中的绝对主体（中国口径约 80%，全球 GBD 口径约 60~70%，两者统计范围不同）；卒中致死率居我国死因第一位" },
  { num: "≥150分钟", color: "var(--orange)", label: "每周中等强度有氧运动最低量", note: "WHO 推荐每周 ≥150 分钟中等强度，或 ≥75 分钟高强度；可拆成每天 10~15 分钟的小段累加" },
  { num: "4分钟", color: "var(--green)", label: "心肺复苏黄金时间", note: "超过6分钟即使抢救成功，也可能造成不可逆脑损伤" },
  { num: "4.5小时", color: "var(--teal)", label: "静脉溶栓的<strong>标准</strong>时间窗", note: "⚠️ 这不是「过时就没救」的截止线：经影像/AI 半暗带筛选，溶栓窗最长可延至 24 小时（中国卒中学会再灌注治疗指南 2024），大血管闭塞取栓窗同样是 24 小时（Ⅰ类推荐 A 级证据）。<strong>不论发病多久、哪怕睡醒才发现，都要立刻打 120</strong>——能不能治由医院的影像检查决定，不由你算的钟点决定" },
...
/* ---------- 2. FAST + BE ---------- */
const fastItems = [
  { l: "F", bg: "var(--blue)", t: "Face 脸", b: "让患者微笑或龇牙，看是否一侧嘴角下垂、眼睑闭合不全——面部不对称是卒中最容易被旁人一眼看出的信号" },
...
document.getElementById("fastgrid").innerHTML = fastItems.map(f => `
  ...
  <div class="fastcard" style="grid-column:1/-1">
    ...
    <div>
      ...
      <div class="fast-ext"><strong>BE FAST扩展识别：</strong>在 FAST 四项基础上加入 Balance（平衡）和 Eyes（视力）两项，目的是补上单纯 FAST 容易漏掉的后循环卒中——这类卒中常以眩晕、站不稳、复视起病，不一定有口角歪斜。</div>
    </div>
  </div>`;
...
const cprSteps = [
  ...
  },
  {
    ...
    ev: "<strong>无人按压时</strong>，每延误 1 分钟，生存机会下降约 7~10%；而只要有人持续做胸外按压，这个下降速度大约能减半——<strong>你按下去的每一分钟，都在实实在在地把时间买回来</strong>。"
  },
];
...
/* ---------- 5. 关键筛查数字 ---------- */
const screens = [
  { icon: "🩸", bg: "var(--red-soft)", t: "血压（中国标准）", v: "正常 &lt;120/80；<strong>正常高值 120–139 或 80–89</strong>（不是高血压，但要开始管生活方式）；<strong>高血压诊断线：诊室 ≥140/90</strong>（家庭自测 ≥135/85，24h 动态 ≥130/80）。降压目标：先到 &lt;140/90，能耐受再往 &lt;130/80 走", n: "依据《中国高血压防治指南（2024年修订版）》——诊断标准维持 140/90 不变。网上常见的「130/80 就算高血压」是美国 2017 年标准，中国未采用。18岁以上都应筛查；40岁以上或高危者至少每年测；&gt;180/120 且伴胸痛/神经症状=高血压急症，立即就医" },
...
const groups = [
  ...
  },
  {
    ...
    blocks: [
      ...
      { h: "🏃 运动", rows: ["有氧运动每周≥5次，每次30~50分钟中等强度", "抗阻运动每周2~3次；工作间隙「微运动」每小时起身5分钟", "运动强度：初始50~70%最大心率（HRmax=207-0.7×年龄，Gellish 公式，比常见的「220-年龄」更准）", "⚠️ 任何按年龄推算的最大心率个体误差都可达 ±10~20 次/分，只能当参考区间，别当精确值；以「能说话但唱不了歌」的主观感受交叉验证"] },
      { h: "🩺 定期筛查", rows: ["30岁及以上每年至少一次血压检测", "年度体检加做心电图、血脂、血糖、同型半胱氨酸检查", "家族史者应做超声心动图、动态心电图检查", "控制体重（我国18岁以上超重率34.3%、肥胖率16.4%，《中国居民营养与慢性病状况报告（2020年）》）"] },
